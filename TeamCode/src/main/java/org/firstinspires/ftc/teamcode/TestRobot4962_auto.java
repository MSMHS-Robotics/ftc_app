 ...
 *
 */
@TeleOp(name = "Test Robot 4962 - auto test", group = "Concept")
//@Disabled
...
//		sleep(10000);
		...
		DriveOnHeading(yawPIDResult,0,12);
		TurnToHeading(yawPIDResult, -45., 2.0);
        DriveOnHeading(yawPIDResult,-45,40);
		...
		if(colorIsBlue()) {
			robot.blueButton.setPosition(0.5);
			DriveOnHeading(yawPIDResult,0,1);
			robot.StopDriving();
		}
		else if (colorIsRed()) {
		...
		robot.blueButton.setPosition(0);

		sleep(2000);

        DriveOnHeading(yawPIDResult,1,42);
		robot.StopDriving();
		readColor();
		...
		if(colorIsBlue()) {
			robot.blueButton.setPosition(0.6);
			DriveOnHeading(yawPIDResult,0,2);
			robot.StopDriving();
		}
		else if (colorIsRed()) {
			DriveOnHeading(yawPIDResult,0,3);
